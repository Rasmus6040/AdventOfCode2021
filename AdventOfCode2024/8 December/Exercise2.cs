    ...
    public long GetResult()
    {
        ...
        foreach (var signals in dict)
        {
            ...
            foreach (((int, int), (int, int)) pair in antennaPairs)
            {
                ...
                for (int i = 0; i < 50; i++)
                {
                    antinodes.Add((pair.Item1.Item1+i*diff.Item1, pair.Item1.Item2+i*diff.Item2));
                    antinodes.Add((pair.Item2.Item1-i*diff.Item1, pair.Item2.Item2-i*diff.Item2));   
                }
            }
        }
        var antenna = antinodes.Where(x => 
                x.Item1 >= 0 && x.Item1 < _puzzleInput.Length && 
                x.Item2 >= 0 && x.Item2 < _puzzleInput.First().Length).ToList();

        foreach (var a in antenna.GroupBy(x => x.Item1 + ";"+x.Item2))
        {
        ...
        return antenna.GroupBy(x => x.Item1 + ";"+x.Item2).Count();
    }
}