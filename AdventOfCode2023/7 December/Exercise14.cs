...
public class Exercise14
{
    ...
    {
        var convertedHand = string.Join("",
            hand.Where(ch => ch != 'J').Select(ch => hand.Count(ch2 => ch2 == ch))
                .OrderByDescending(ch => ch)
                .Select(ch => ch.ToString()));
        
        if (convertedHand.Length == 0) return 55555;
        
        if (convertedHand.Length == 5) return int.Parse(convertedHand);
        
        var firstDigit = int.Parse(convertedHand.First().ToString());
        var newDigit = firstDigit + 5 - convertedHand.Length;
        return int.Parse(convertedHand.Remove(0, firstDigit).Insert(0, new string(newDigit.ToString().First(), newDigit)));
    }
}